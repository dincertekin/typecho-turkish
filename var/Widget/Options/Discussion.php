...
class Discussion extends Options implements ActionInterface
{
    ...
    public function form(): Form
    {
        ...
        $commentDateFormat = new Form\Element\Text(
            ...
            . _t('Lütfen PHP gün formatı hakkında daha fazla bilgi için <a href="http://www.php.net/manual/zh/function.date.php">bu dökümanı</a> inceleyiniz.')
        );
        ...
        $commentsShowOptions = [
            ...
            'commentsUrlNofollow'     => _t('Yorumcuların kişisel sayfalarına bağlantılar için <a href="https://en.wikipedia.org/wiki/Nofollow">nofollow</a> özelliği kullanılsın'),
            'commentsAvatar'          => _t('<a href="https://gravatar.com">Gravatar</a> avatar hizmetini etkinleştirin, maksimum görüntüleme derecesi %s avatarlar',
                '</label><select id="commentsShow-commentsAvatarRating" name="commentsAvatarRating">
            <option value="G"' . ('G' == $this->options->commentsAvatarRating ? ' selected="true"' : '') . '>' . _t('G - Genel') . '</option>
            <option value="PG"' . ('PG' == $this->options->commentsAvatarRating ? ' selected="true"' : '') . '>' . _t('PG - 13+ Yaş') . '</option>
            <option value="R"' . ('R' == $this->options->commentsAvatarRating ? ' selected="true"' : '') . '>' . _t('R - 17+ Yaş') . '</option>
            <option value="X"' . ('X' == $this->options->commentsAvatarRating ? ' selected="true"' : '') . '>' . _t('X - Kısıtlı') . '</option></select>
